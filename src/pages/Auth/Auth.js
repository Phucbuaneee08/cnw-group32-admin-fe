...
function Auth() {
  ...
  return (
    <div className="relative bg-homepage-background bg-cover  w-full  flex items-center justify-center py-32  ">
      <div className="w-md space-y-8">
        <div>
          ...
          <h2 className="text-center text-3xl font-bold text-gray-900 pt-2">
            Hệ thống quản lý JadeHill Homestays
          </h2>
        </div>
        <div className="w-full bg-white p-8 border border-gray-200 rounded-lg shadow-md">
          <form className="space-y-6" method="POST">
            ...
            <div>
              <button
                type="submit"
                className="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-blue-700 shadow-sm hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-600"
                onClick={authHandler}
              >
                <span className="absolute left-0 inset-y-0 flex items-center pl-3">
                  {loading ? (
                    <div className="border-top-transparent w-5 h-5 border-4 border-blue-400 border-solid rounded-full animate-spin" />
                  ) : (
                    <LockClosedIcon
                      className="h-5 w-5 text-blue-600 group-hover:text-blue-900"
                      aria-hidden="true"
                    />
                  )}
                </span>
